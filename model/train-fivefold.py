...
def main():
    ...
    for fold in range(1, 6):
        ...
        for epoch in range(num_epochs):
            ...
            if val_auc > best_val_auc:
                ...
                early_stop_count += 1
                print(f'  No improvement in validation AUC for {early_stop_count} epoch(s)')

            # 早停检查
            if early_stop_count >= patience:
                print(f'Early stopping after {epoch+1} epochs due to no improvement in validation AUC for {patience} consecutive epochs')
                break

        # 记录当前fold的最佳验证AUC
        fold_results.append(best_val_auc)
        print(f'Fold {fold} best validation AUC: {best_val_auc:.4f}')
    ...
    print(f'Final Test Results with Ensemble: Acc={accuracy:.4f}, Prec={precision:.4f}, Rec={recall:.4f}, F1={f1:.4f}, AUC={auc:.4f}')
